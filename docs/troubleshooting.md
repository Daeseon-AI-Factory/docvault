...
- **Fix**: Commit 25462c3d3dc73ac1a4e434a74c6498670d78151a. Added `thoughtSignature` to `gmPart` and `Sig` to `ToolCall` (`internal/agent/agent.go`, `internal/agent/providers.go`): capture `part.thoughtSignature` into `ToolCall.Sig` on parse, and re-emit it on the model-role functionCall part when rebuilding `contents`. OpenAI ignores `Sig`. Verified end-to-end: with `DOCVAULT_AI_PROVIDER=gemini`, a tool-using chat now completes instead of 400ing.
