...
- **Fix**: Commit 25462c3d3dc73ac1a4e434a74c6498670d78151a. Added `thoughtSignature` to `gmPart` and `Sig` to `ToolCall` (`internal/agent/agent.go`, `internal/agent/providers.go`): capture `part.thoughtSignature` into `ToolCall.Sig` on parse, and re-emit it on the model-role functionCall part when rebuilding `contents`. OpenAI ignores `Sig`. **Partial live verification**: after deploying with `DOCVAULT_AI_PROVIDER=gemini`, the `400 thought_signature` error no longer occurs — the live error changed to a transient `gemini 503: high demand` (Google capacity/throttle on this key), so the malformed-request cause is resolved, but a clean 200 through Gemini was not obtainable during testing because Google was returning 503 for both `gemini-flash-latest` and `gemini-2.5-flash`. End-to-end-200 on Gemini is therefore unconfirmed; the demo runs on OpenAI (verified 200) for reliability. Retry Gemini when Google capacity recovers.
