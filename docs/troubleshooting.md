...
<!-- skipped: 25035d1 Log demo-login hardening + code-signing pipeline [no-log] -->
<!-- skipped: e67c0e3 Blog log: adversarial review of Codex changes + demo-login hardening retro [no-log] -->
<!-- skipped: 50a5ba8 Add Korean study guides: beginner (zero-jargon, term-by-term + stack tradeoffs) + interview-ready [no-log] -->

## AI assistant couldn't guide a non-technical admin (only answered live data)

- **Symptom**: the operator (a non-technical admin running DocVault for their company) found the web UI too hard, and the AI assistant couldn't help — asked "how do I install on an employee PC?" or "how do I register a user?", it had no way to answer. The agent only had live-data tools (`event_counts`, `recent_events`, `list_alerts`, `list_users`, `list_hosts`) plus action tools; nothing for product how-to, so usage questions either floundered or risked being made up.
- **Cause**: the assistant's tools and system prompt were scoped to querying/acting on live data. There was no usage/how-to knowledge for it to ground on (usage docs like `manual.ko.html` existed but the agent never read them).
- **Fix**: Commit c3c2c69973bcfb873b5aa1d09afd89987f1cc7b4. Added a doc-grounded help capability — a curated usage guide `internal/agent/help_kb.md` (embedded) and a `help_docs` tool (`internal/agent/help.go`) that returns it, wired into `NewEngine`. System prompt now instructs: for usage/install/how-to questions, call `help_docs` first and answer step-by-step in simple Korean **grounded only in that text**, saying "not in the guide" otherwise. Dashboard AI box now invites how-to questions. This is lightweight RAG: the tool is the retrieval step over a small KB; at this size returning the whole guide is more accurate than vector retrieval (no retrieval miss) — upgrade to embeddings only if the KB outgrows the context window.
- **Commit**: c3c2c69973bcfb873b5aa1d09afd89987f1cc7b4
- **Pattern**: for how-to/usage answers, ground the LLM on a curated doc (retrieval) instead of letting it freestyle. Don't reach for a vector DB until the KB no longer fits in context — "RAG" is retrieve-then-generate, and for a small KB the retrieval can just be "return the curated guide".
<!-- skipped: 0416038 Backfill 6 foundation blog entries from git history (Mar 31 - Apr 8) [no-log] -->

## Important issues only visible in-app — admin never got notified

- **Symptom**: the alert engine created alerts (11 active rules, evaluated on every event) and the dashboard showed them, but a non-technical admin who doesn't log in daily would never *know* an important issue happened. The only push channel was Slack (`internal/alert/notifier.go`), and on prod `DOCVAULT_SLACK_WEBHOOK` was empty — so nothing was pushed anywhere. `DOCVAULT_ALERT_EMAIL` existed in config but no email was ever implemented or sent.
- **Cause**: notifications were pull-only (dashboard) plus a Slack-only notifier that was unconfigured; email was a config field with no code behind it.
- **Fix**: Commit 383266460a91d85f48512b913bc4bd5f66be365b. Added SMTP email via stdlib `net/smtp` (`internal/alert/email.go`) — no dependency, STARTTLS on 587 (works with a Gmail app password). The notifier now emails **high/critical** alerts immediately (severity-gated to avoid inbox spam). Added a **daily plain-Korean digest** (`internal/alert/digest.go`) — 24h event counts + unacked alerts — scheduled at 09:00 Asia/Seoul (`cmd/server/main.go`, waits to the next scheduled hour so a redeploy doesn't fire one). Admin-only `POST /api/alerts/digest-now` sends it on demand. New config: `DOCVAULT_SMTP_HOST/PORT/USER/PASS/FROM`, `DOCVAULT_PUBLIC_URL`, `DOCVAULT_DIGEST_HOUR`; all email features are off unless SMTP host + from + to are set.
- **Commit**: 383266460a91d85f48512b913bc4bd5f66be365b
- **Pattern**: a monitoring tool that only shows issues when you log in doesn't notify — it waits. Pair in-app surfacing with a proactive push the operator actually checks (email beats Slack for a small Korean team), and gate per-event email by severity while a daily digest carries the rest. Note: cloud providers sometimes block outbound SMTP (587) — verify the server itself can send, not just your laptop.

## Portfolio demo showed a different (crippled) product than the real one

- **Symptom**: the public demo looked nothing like the real product — no AI assistant, no AI briefing, no admin menus. An earlier safety fix had made the demo login a NON-admin (manager) user, and all of those features are admin-only (`{{if eq .Role "admin"}}` + `auth.RequireRole("admin")`), so the demo was a stripped half-product — useless as a portfolio showcase.
- **Cause**: the demo-login hardening (refuse admin role) traded away demo completeness for safety. Correct instinct (don't grant passwordless admin) but wrong mechanism for a showcase.
- **Fix**: Commit 6773e7f75d88dd42256bfaf23f270ddb30af6956. Made the demo user an **admin** again (identical screens + AI), and made the demo **read-only** instead: `web.DemoReadOnly` middleware (on both protected groups) blocks every mutating request for the demo user — allowing reads and the AI chat only — and the assistant runs on a **read-only engine** (`agent.NewReadOnlyEngine`, no action tools) when the actor is the demo user. So the demo shows the whole product but a visitor can neither change anything nor make the AI act. Safe even if `DEMO_LOGIN_ENABLED` were ever misconfigured (worst case = read-only view). Demo box reseeds the `demo` user to admin.
- **Commit**: 6773e7f75d88dd42256bfaf23f270ddb30af6956
- **Pattern**: for a public demo, "least privilege" isn't "lower role" (that hides the product) — it's **full visibility + read-only enforcement**. Block mutations at the middleware AND remove the AI's action tools, since AI actions execute server-side and bypass HTTP method checks.

## Demo AI failed (502) — base compose didn't pass AI keys + Gemini tool-use is broken

- **Symptom**: on the demo box the AI assistant returned 502; server log: `gemini 400: Function call is missing a thought_signature in functionCall parts ... function call default_api:list_users`. Before that it was 503 (engine disabled) because the demo container had no AI key at all.
- **Cause**: two issues. (1) The base `docker-compose.yml` `&app-env` anchor never passed `DOCVAULT_GEMINI_API_KEY`/`OPENAI_API_KEY` to the server, so the demo stack ran with AI disabled (prod works only because its separate `/opt/mimi` override passes them). (2) Google's Gemini API now **requires a `thought_signature` on `functionCall` parts** in multi-turn tool-use; our `GeminiProvider` doesn't send it, so every tool-use round-trip 400s. Prod dodged this by using OpenAI (provider precedence openai > gemini).
- **Fix**: added the AI-key passthrough to the base compose `&app-env` anchor (commit on this branch) so any stack (incl. demo) can enable AI via its `.env`. For the demo, configured it to use **OpenAI** (same working provider as prod) via `.env.demo` — demo AI now answers (read-only). The Gemini `thought_signature` gap in `internal/agent/providers.go` is a **known latent bug** (only bites when Gemini is the active provider for tool-use); not yet fixed.
- **Pattern**: a shared base compose must expose every optional integration's env, or one deployment silently runs without it. And LLM function-calling protocols drift — Gemini added a required `thought_signature`; pin to a provider you've verified end-to-end for tool-use, and treat the others as untested until proven.
<!-- skipped: 104ce73 Base compose passes AI keys (so demo can enable AI); note Gemini tool-use bug [no-log] -->

## Gemini tool-use 400: missing thought_signature (now fixed)

- **Symptom**: with Gemini as the AI provider, every tool-using chat failed: `gemini 400: Function call is missing a thought_signature in functionCall parts. This is required for tools to work correctly ... function call default_api:list_users`. (Previously documented as a latent bug; prod/demo dodged it by using OpenAI.)
- **Cause**: Gemini attaches an opaque `thoughtSignature` to each `functionCall` part it returns, and **requires that same signature to be echoed back** when the call is replayed in the next turn (the tool-use loop sends the model's functionCall + our functionResponse on the follow-up request). `GeminiProvider.Chat` parsed only `name`+`args` and dropped the signature, so the replay had none → 400.
- **Fix**: Commit 25462c3d3dc73ac1a4e434a74c6498670d78151a. Added `thoughtSignature` to `gmPart` and `Sig` to `ToolCall` (`internal/agent/agent.go`, `internal/agent/providers.go`): capture `part.thoughtSignature` into `ToolCall.Sig` on parse, and re-emit it on the model-role functionCall part when rebuilding `contents`. OpenAI ignores `Sig`. **Verified live** via `internal/agent/gemini_live_test.go` (a re-runnable test, skipped without a key): turn 1 returns a `functionCall` with `thoughtSignature` captured (`sig present=true`); turn 2 — replaying that call, the exact request that returned `400` before the fix — now succeeds with a real answer (`현재 이벤트 개수는 42개입니다.`), so the full Gemini tool-use loop completes. (An initial round of testing hit a transient `gemini 503: high demand`; once Google's capacity recovered the 200 went through.) The demo still runs on OpenAI for reliability.
- **Commit**: 25462c3d3dc73ac1a4e434a74c6498670d78151a
...
<!-- skipped: 9cb6b1e Log Vultr migration: missing shared-Caddy vhost was the outage, deploy script retargeted [no-log] -->
<!-- skipped: fdee4fa Merge pull request #24 from Daeseon-AI-Factory/fix/deploy-vultr-box -->
